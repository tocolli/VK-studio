...
async function registrar(req, res) {
  try {
    ...
    if (senha.length < 6) return res.status(400).json({ success: false, message: 'Senha deve ter no mínimo 6 caracteres.' });
    const [existente] = await pool.execute('SELECT id FROM users WHERE email = ?', [email]);
    if (existente.length > 0) return res.status(409).json({ success: false, message: 'E-mail já cadastrado.' });
    ...
    return res.status(201).json({ success: true, message: 'Conta criada com sucesso!', token, usuario: { id: usuario.id, nome, email: usuario.email, role } });
  } catch (err) {
...
async function perfil(req, res) {
  try {
    const [rows] = await pool.execute(
      'SELECT id, nome, email, role, avatar_url, created_at FROM users WHERE id = ?', [req.user.id]
    );
...
// NOVO: Atualizar avatar do usuário
async function atualizarAvatar(req, res) {
  try {
    const avatar_url = req.file?.path;
    if (!avatar_url) return res.status(400).json({ success: false, message: 'Nenhuma imagem enviada.' });
    await pool.execute('UPDATE users SET avatar_url = ? WHERE id = ?', [avatar_url, req.user.id]);
    // Retorna novo token com dados atualizados (avatar não está no token, mas atualiza session)
    return res.json({ success: true, message: 'Avatar atualizado!', avatar_url });
  } catch (err) {
    return res.status(500).json({ success: false, message: 'Erro interno.' });
  }
}

// NOVO: Listar todos os usuários (só mestre)
async function listarUsuarios(req, res) {
  try {
    const [rows] = await pool.execute(
      'SELECT id, nome, email, role, avatar_url, created_at FROM users ORDER BY created_at DESC'
    );
...
module.exports = { registrar, login, perfil, atualizarAvatar, listarUsuarios };
